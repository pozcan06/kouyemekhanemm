...
function loadMenu(dateStr) {
    ...
    // Show delete menu button only for logged-in users
    if (deleteMenuBtn) {
        deleteMenuBtn.style.display = currentUser ? "inline-flex" : "none";
    }
...
function buildFoodCard(foodItem, cat, dateStr) {
    ...
    let imageHTML = "";
    if (foodItem.image) {
        const photoDeleteBtn = currentUser 
            ? `<button class="btn-delete-photo" title="Fotoğrafı sil" data-category-key="${cat.key}" data-date="${dateStr}">
                <i class="fa-solid fa-trash-can"></i>
               </button>`
            : "";
        imageHTML = `<img src="${foodItem.image}" alt="${foodItem.name}" loading="lazy">${photoDeleteBtn}`;
    } else {
    ...
    // Add event listener to photo delete button (if present)
    const photoDeleteBtn = card.querySelector(".btn-delete-photo");
    if (photoDeleteBtn) {
        photoDeleteBtn.addEventListener("click", (e) => {
            e.stopPropagation();
            openConfirmModal(
                "Fotoğrafı Sil",
                `"${escapeHtml(foodItem.name)}" yemeğinin fotoğrafını silmek istediğinize emin misiniz?`,
                () => deletePhoto(dateStr, cat.key)
            );
        });
    }
...
// --- MENU DELETE ---
function setupDeleteMenuBtn() {
    const btn = document.getElementById("delete-menu-btn");
    if (!btn) return;
    ...
    btn.addEventListener("click", () => {
        ...
        openConfirmModal(
            "Menüyü Sil",
            `${document.getElementById("day-display-name").textContent} tarihine ait tüm menüyü silmek istediğinize emin misiniz? Bu işlem geri alınamaz.`,
            () => deleteMenu(currentSelectedDate)
        );
    });
}

function deleteMenu(dateStr) {
    if (!dateStr) return;

    dbRef.child('menus').child(dateStr).remove()
        .then(() => {
            showToast(`${formatVisualDate(dateStr)} menüsü başarıyla silindi.`, "success");
            // loadMenu will be called automatically by Firebase real-time listener
        })
        .catch((err) => {
            console.error("Menu delete error:", err);
            showToast("Menü silinirken bir hata oluştu.", "error");
        });
}

// --- PHOTO DELETE ---
function deletePhoto(dateStr, categoryKey) {
    if (!dateStr || !categoryKey) return;

    dbRef.child('menus').child(dateStr).child(categoryKey).child('image').set("")
        .then(() => {
            showToast("Fotoğraf başarıyla silindi.", "success");
            // UI will refresh via Firebase real-time listener
        })
        .catch((err) => {
